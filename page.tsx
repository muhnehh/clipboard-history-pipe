...
export default function Page() {
  ...
  const explainClipboardItem = (content: string) => {
    if (/^https?:\/\//i.test(content)) {
      return 'This is a URL link';
    } else if (content.length > 50) {
      return `Long text (${content.split(' ').length} words)`;
    } else if (/\d/.test(content)) {
      return 'Contains numbers';
    } else {
      return 'Short text snippet';
    }
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (document.hasFocus() && hasPermission) {
        try {
          ...
          if (content && !history.some(item => item.content === content)) {
            ...
            const newEntry: ClipboardItem = {
              ...
              explanation: explainClipboardItem(content),
            };
  ...
  return (
    <ClientOnly>
      <div style={{
        ...
        <h2 style={{ fontSize: '28px', fontWeight: '600', color: '#1d1d1f', marginBottom: '10px' }}>
          Clipboard History Pipe
        </h2>
        <p style={{ fontSize: '16px', color: '#86868b', marginBottom: '20px', textAlign: 'center' }}>
          {hasPermission
            ? "Click an entry to copy it back. Powered by Screenpipe context."
            : "Clipboard access required. Allow permissions to start tracking."}
        </p>
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                  ...
                  <div style={{ marginTop: '5px', fontSize: '12px', color: '#86868b' }}>
                    ...
                    {item.url && (
                      <span> | <a href={item.url} style={{ color: '#007aff', textDecoration: 'none' }} target="_blank" rel="noopener noreferrer">Source URL</a></span>
                    )}
                    {item.suggestion && (
                      <span> | Suggestion: <span style={{ color: '#1d1d1f' }}>{item.suggestion}</span></span>
                    )}
                    <span> | Type: <span style={{ color: '#1d1d1f' }}>{item.explanation}</span></span>
                  </div>
                </div>
              </li>
            ))
        ...
        <footer style={{ marginTop: '40px', fontSize: '14px', color: '#86868b' }}>
          Built with <a href="https://screenpi.pe" style={{ color: '#1d1d1f', textDecoration: 'underline' }}>Screenpipe</a>
        </footer>
      </div>
    </ClientOnly>
  );
